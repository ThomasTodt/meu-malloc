...
alocaMemoria:
        movq %rbx, %r8
        addq $16, %r8
        addq 8(%rbx), %r8      # r8 aponta pro proximo de rbx

        # adiciona a flag de ocupado e o tamanho de memória
		movq $1, 0(%rbx)    # bloco indicado por rbx esta ocupado agora
		movq %r9, 8(%rbx)  # bloco de tamanho pedido pelo programador

		# atualiza o endereço de parada da ultima alocação
        movq %rbx,    %r10     # r10 = rbx
        addq $16,     %r10     # r10 aponta pro primeiro byte de conteudo
        movq %rbx,    %r11
        addq $8,      %r11
        movq (%r11),  %r11
        addq %r11,    %r10     # r10 = header do proximo bloco 
        ...
        jl atualiza_parada      #   parada = proximo bloco
        ...
        atualiza_parada: # cria header do proximo se tiver espaco de 16 bytes
        # movq $0,   (%r10)
        ...
        addq %r8, 8(%rbx)         # se menor que 16 adiciona os que sobraram no alocado 
        movq %rbx, %rcx
        ...
        fim_alocacao:
            movq %rbx, %rax           # retorna endereco do bloco que agora esta ocupado  
...
aumenta_heap:
    ...
    jge fim_procura_ultimo    # entao achou o ultimo  
    ...
    subq $16,  %r8
    movq %r8,  8(%r10)
    # movq $84, 8(%r10)         # tamanho do novo bloco (100 - 16 (header)) 
    ...
    fim_aumenta_heap:
    jmp verificaAlocacao      # depois de aumentar a heap, repete o processo todo
...
liberaMem:
    ...
    addq $8,     %rbx           # bytes do "tamanho" (long)
